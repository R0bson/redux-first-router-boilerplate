...
const createTestLink = children => shallow(
  <Component href="http://some.test/link">{children}</Component>
).dive()

const createCenteredTestLink = children => shallow(
  <Component center href="http://some.test/link">{children}</Component>
).dive()
...
describe('OutboundLink', () => {
  ...
  describe('rendering centered', () => {
    ...
    it('should render an "a" component in div', () => {
      const wrapper = createCenteredTestLink('Test link')
      expect(wrapper.childAt(0).dive().prop('component')).toBe('a')
    })
    ...
    it('should pass href prop', () => {
      const wrapper = createCenteredTestLink('Test link')
      expect(wrapper.childAt(0).dive().prop('href')).toBe('http://some.test/link')
    })

    it('should have noopener tag', () => {
      const wrapper = createCenteredTestLink('Test link')
      expect(wrapper.childAt(0).dive().prop('rel').split(/\s/)).toContain('noopener')
    })

    it('should have noreferrer tag', () => {
      const wrapper = createCenteredTestLink('Test link')
      expect(wrapper.childAt(0).dive().prop('rel').split(/\s/)).toContain('noreferrer')
    })

    it('should target _blank', () => {
      const wrapper = createCenteredTestLink('Test link')
      expect(wrapper.childAt(0).dive().prop('target')).toBe('_blank')
    })
  })
})
